...
class ProductsSearchPage extends BasePage{
    ...
    get productCard() {
         return (productName) => cy.get(`#content .product-thumb img[title='${productName}']`).parents('.product-thumb'); 
        }
    get alert() {
    ...
    get productDescription() {
         return (productName) => this.productCard(productName).find('.caption p').first()
        }
    get productPrice() { 
        return (productName) => this.productCard(productName).find('.caption .price')
    }
